...
public class GetRouletteSessionDetailsQueryHandler(ISessionsRepository repository, IMapper mapper) 
    : IRequestHandler<GetRouletteSessionDetailsQuery, RouletteSessionResultViewModel>
{
    public async Task<RouletteSessionResultViewModel> Handle(GetRouletteSessionDetailsQuery request, CancellationToken cancellationToken)
    {
